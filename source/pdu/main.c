...
-------------------------------------------------------- */
//TODO MCG: fix this math With 11 items, 16 prescaler, and 640 sample time, each channel gets read every 1.4ms
...
-------------------------------------------------------- */
dma_init_t adc_dma_config = ADC1_DMA_CONT_CONFIG((uint32_t) &adc_readings, sizeof(adc_readings) / sizeof(adc_readings.lv_24_v_sense), 0b01);
...
int main()
{
    ...
    taskCreate(sendIVReadings, 500);
    taskCreate(checkSwitchFaults, 100);
    taskCreate(sendFlowrates, 200);
...
void heatBeatLED()
{
    ...
    if (trig)
    {
        SEND_PDU_CAN_STATS(can_stats.tx_of, can_stats.tx_fail, can_stats.rx_of, can_stats.rx_overrun);
    }
    trig = !trig;
 ...
 * 
 * @brief sends CAN message with flow rates
 */
void sendFlowrates()
{
